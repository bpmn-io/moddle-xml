...
ElementSerializer.prototype.build = function(element) {
  ...
  if (element.$descriptor.isGeneric) {
    ...
    this.parseGenericAttributes(element);
  }
...
ElementSerializer.prototype.parseGenericAttributes = function(element) {
  ...
  _.forEach(otherAttrs, function(value, name) {
    var nameNs = parseNameNs(name);

    try {
      self.logNamespaceUsed(nameNs);
      self.addAttribute(name, value);
    } catch (e) {
      console.warn('[writer] missing namespace information for ', name, '=', value, 'on', element, e);
    }
  });
...
ElementSerializer.prototype.getNamespaces = function() {
  if (!this.parent) {
    ...
    this.ns = this.parent.getNamespaces();
  }
...
ElementSerializer.prototype.logNamespaceUsed = function(ns) {
  ...
  var uri = ns.uri || DEFAULT_NS_MAP[prefix] || namespaces.prefixMap[prefix] || (model ? (model.getPackage(prefix) || {}).uri : null);
...
ElementSerializer.prototype.parseAttributes = function(properties) {
  ...
  _.forEach(properties, function(p) {
    ...
    self.addAttribute(p.name, value);
  });
};
...
ElementSerializer.prototype.serializeAttributes = function(writer) {
  ...
  function collectNsAttrs() {
    return _.collect(namespaces.used, function(ns) {
      return { name: 'xmlns:' + ns.prefix, value: ns.uri };
    });
  }
